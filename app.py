...
def main():
    ...
    activity = ["eda","predictor","countries","metrics","about"]
    ...
    if choice == 'eda':
        ...
        if st.checkbox("Preview Dataset"):
            number = int(st.number_input("Number to Show", 5))
    ...
    # PREDICTION
        ...
        relationship = st.selectbox("Select Education", tuple(d_relationship.keys()))
        race = st.selectbox("Select Race", tuple(d_race.keys()))
        sex = st.radio("Select Gender",tuple(d_sex.keys()))
        capital_gain = st.number_input("Capital Gain",-1,99999)
        capital_loss = st.number_input("Capital Loss", -1, 4356)
